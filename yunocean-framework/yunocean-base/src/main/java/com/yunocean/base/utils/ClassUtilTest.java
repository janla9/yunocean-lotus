...
public class ClassUtilTest 
{
	public static void main(String[] args) {
		Set<Class<?>> clazzs = ClassUtil.scanPackage("com.yunocean");
		if(clazzs == null) return;
		for (Class<?> clazz : clazzs) {
			//先或者此类的API注解名称
			Api apiClazz = clazz.getAnnotation(Api.class);
			if(apiClazz == null) continue;
			if(apiClazz.tags().length < 1) continue;
			
			System.out.println("父名称：" + apiClazz.tags()[0]);
			// 获取方法上的注解
	...
	public static void main2(String[] args) {
        ...
        // 包下面的类
        Set<Class<?>> clazzs = getClasses("com.yunocean");
        if (clazzs == null) {
            return;
        }
